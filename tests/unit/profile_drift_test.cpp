...
{
    scan found{ {}, 0 };
    std::ifstream in{ std::filesystem::path{ MEIOS_DOCS_DIR } / "urdf-profile.md" };
    std::string line;
...
TEST_CASE("the profile document carries one rule marker per rule it publishes", "[profile][drift]")
{
...
TEST_CASE("the profile document and the rule table name the same rules", "[profile][drift]")
{
...
TEST_CASE("every diagnostic code the rule table names round-trips through to_string",
          "[profile][drift]")
{
