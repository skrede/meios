...
// Wraps the rewrite/collision/closure layer into the emitter's deferred rewrite
// hook: it feeds every accumulated reference into the shared builder, drives the
// transitive scanner closure, then hands back the original->rewritten table.
...
asset_manifest bundle_impl(const Robot &robot, source_stack &sources, scanner_registry &registry,
                           const folder_request &request, emit_result &out, log_sink &log)
{
